...
When the plugin starts, `_inject_proxy_env()` sets **all** secret environment
variables to the sentinel value `proxy-a0`. An embedded auth-proxy at
`127.0.0.1:{port}` intercepts outbound LLM API calls and transparently
...
```python
from deimos_openbao_secrets.helpers.factory_common import resolve_secret
